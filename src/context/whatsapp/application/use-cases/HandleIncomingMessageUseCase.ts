...
export class HandleIncomingMessageUseCase {
  ...
  async execute(command: IncomingMessageCommand): Promise<Message> {
    ...
    // Dedupe by key.id when present
    if (command.key?.id) {
      const existing = await this.messages.findByKeyId(command.key.id);
      if (existing) {
        return existing;
      }
    }

    const message = Message.createIncoming(
      command.sessionId,
      ...
      command.key?.id
        ? {
            id: command.key.id,
            remoteJid: command.key.remoteJid ?? '',
            fromMe: Boolean(command.key.fromMe),
          }
        : undefined,
    );
